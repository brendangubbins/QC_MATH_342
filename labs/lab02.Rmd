...
i = 1
for (i in 1:8) {
  arrays[[LETTERS[i]]] = array(data = 1:i^i, dim = c(rep(i, times = i)))
}
...
The bytes are increasing exponentially. This makes sense because there is an entirely new dimension being added for each key/array.
...
rm(my_list,arrays)
#rm(list = ls())
# NOTE I COMMENTED ABOVE BECAUSE IT WILL REMOVE my_reverse which is needed later
```
...
split = strsplit(lorem, split = ". ", fixed = TRUE)
lorem = unlist(split)
lorem = sample(lorem)
...
men$boomers = strsplit("Theodore, Bernard, Gene, Herbert, Ray, Tom, Lee, Alfred, Leroy, Eddie", split = ", ")[[1]]
...
women$boomers = strsplit("Gloria, Joan, Dorothy, Shirley, Betty, Dianne, Kay, Marjorie, Lorraine, Mildred", split = ", ")[[1]]
...
X = data.frame(treatment = rep(NA,n), variation = rep(NA,n), gender = rep(NA,n), generation = rep(NA,n),
               name = rep(NA,n), y = rep(NA,n))

X$treatment = rep(letters[1:14], each = n/14)
X$variation = rep(rep(LETTERS[1:3], each = n/14/3))
X$gender = rep(rep(c("M","F"), each = n/14/3/2))
X$generation = rep(c("Boomer", "GenX", "Millennial"), each = n/14/3/2/3)
X$name = rep(unlist(name_list))
...
```{r}
#install.packages("pacman", repos = "https://cran.r-project.org/web/packages/pacman/index.html")
```
...
#'
#' Classifies an observation based on the label of the closest observation in a set of training observations
#'
...
nn_algorithm_predict = function(Xinput, y_binary, xtest){
  ...
  for (i in 1:n) {
    distances[i] = sum((Xinput[i,] - xtest)^2) # squared euclidean distance (don't need to sqrt)
  }
...
nn_algorithm_predict = function(Xinput, y_binary, xtest, d = function(v1, v2){sum((v1 - v2)^2)}){
  ...
  distances = array(NA, n)
  for (i in 1:n) {
    distances[i] = d(Xinput[i,], xtest) # squared euclidean distance (don't need to sqrt)
  }
  y_binary[which.min(distances)]
}
```
...
# add K arg, find K nearest, find modal label
```
...
This dataset describes 150 different iris' (number of rows) with 5 different properties (number of columns). It contains 1 factor and 4 numerics. The factor is the Species property, which is either set (setosa), ver (versicolor), or vir (virginica). Then, each iris has numeric properties such as their Sepal.Length, Sepal.Width, Petal.Length and Petal.Width which measure their sepal and petal sizes. The species are evenly split in the data set (50 of each). 
...
```{r}
y = as.integer(iris$Species == "versicolor") # was setosa in class (mistake?)
```
...
```{r}
n = nrow(iris) # n = 100
num_errors = array(dim = n)
for (i in 1 : n) {
  temp_threshold = iris$Sepal.Length[i]
  g = as.integer(iris$Sepal.Length > temp_threshold)
  num_errors[i] = sum(as.integer(g != y))
}
...
for (i in 1 : n) {
  errors = sum(as.integer(iris$Sepal.Length > threshold) != y)
}
...
This makes sense because the threshold I reached was 5.4, and the median for versicolor is 5.9
whereas the median for setosa is 5.0. The threshold is split in between these so then it will
decently model the two outcomes. 
...
#'
#' For linearly separable data, returns the best possible vector to separate the outputs given
#' a maximum amount of iterations
#'
#' @param Xinput      The input features
#' @param y_binary    Binary output of 1 or 0
#' @param MAX_ITER    Maximum amount of iterations to perform
#' @param w           A vector of weights of features
#'
...
The blue ones are shown in the top right corner and the red zeros are near the bottom left corner.
So, this data is clearly linearly separable, thus a perceptron would be useful here.
...
The output is (b, w0, w1), where b is the bias, w0, w1 are weights. The intercept is -7 and the slope is 4. 
...
This line is not satisfying because it is not cleanly separating the orange and blue data points.
The line is crossing the orange data point at (2,1).
