...
```{r}
#prod(1/(3*(1:37))) there's a better way
...
It's not exactly correct because it is a very small positive number. The number was too small and we experienced numerical underflow so R defaulted to 0.
...
As x approaches 0, log(x) approaches -Inf. So when x = 0 in this case, it shows that log(0) = -Inf. log(0) technically does not exist but the answer is appropriate enough. As x approaches Inf, then log(x) approaches Inf so this is appropriate. Log does not exist for negative values of x, so R gives us a NaN, which is maybe not as appropriate as "NA" since log(-x) is not available/applicable rather than not a number.
...
```{r}
which(!y) #finding the falses in y (non-reals)
# or which(y == FALSE)
```
...
```{r}
as.integer(x) # INFs and NaN converted to NA
```
...
```{r}
mean(sample(c(0,1),size=100,replace=TRUE,prob=c(0.1,0.9)))
```
...
```{r}
inf = as.numeric(x_3_ord == "infraction")
inf
mis = as.numeric(x_3_ord == "misdimeanor")
mis
fel = as.numeric(x_3_ord == "felony")
fel

X = matrix(data=NA,nrow=100,ncol=3)
...
The row represents a person and each of their columns represents if that was the highest crime that they committed. So the sum of the row can only be 1 or 0. This means that the sum of the row represents if an individual committed ANY crime whatsoever.
...
The column sum represents the total number of a certain crime that everyone in the data set has committed. For example, if 20 people committed a felony, then the sum of the felony column is 20.
...
Y = matrix(data=NA, nrow=100,ncol=6)
rownames(Y) = fake_first_names
?rnorm
Y[,1] = rnorm(n=100,mean=17,sd=sqrt(38))
?runif
Y[,2] = runif(n=100,min=-10,max=10)
?rpois
Y[,3] = rpois(n=100,lambda=6)
?rexp
Y[,4] = rexp(n=100,rate=9)
?rbinom
Y[,5] = rbinom(n=20,size=100,p=0.12)
Y[,6] = rbinom(n=100,size=1,p=0.24)
Y
```
...
```{r}
Y_frame = data.frame(Y)
Y_frame[,6] = ifelse(Y_frame[,6] == 1, "FOREIGN", "DOMESTIC")
View(Y_frame)
```
...
```{r}
table(Y_frame[,6])
table(Y_frame$X6, Y_frame$X6) / 100
```

Print out a summary of the whole dataframe.

```{r}
Y_frame$X6 = factor(Y_frame$X6, labels = c("DOMESTIC", "FOREIGN"))
summary(Y_frame)
```
...
```{r}
?order # permutes indices 
R[order(rowSums(R, na.rm = TRUE), decreasing = TRUE),] # (row manipulation) , (nothing in columns)
R # R was unchanged
R = R[order(rowSums(R, na.rm = TRUE), decreasing = TRUE),] # assigning
...
```{r}
?apply
apply(R, MARGIN = 1, sd, na.rm = TRUE) # I don't know why this is all 0s
apply(R, MARGIN = 2, sd, na.rm = TRUE)
# apply(R, MARGIN = c(1,2), sd, na.rm = TRUE) # all NAs
```
...
```{r}
?apply
vec = apply(R > 0, MARGIN = 2, sum, na.rm = TRUE)
vec
```
...
?lapply
?split
lapply(split(R, col(R)), function(X) list(min = min(X, na.rm = TRUE), max = max(X, na.rm = TRUE), pct_missing = sum(is.na(X))/50, first_NA = min(which(is.na(X))))) 
```
...
# The estimate is about the same
```
...
# The estimate is about the same when you take the CDF of 0 of this distribution 0.84134475
```
